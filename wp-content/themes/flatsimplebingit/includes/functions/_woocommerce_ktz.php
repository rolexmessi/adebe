...
function ktz_head_cart() {
	...
	if (class_exists( 'woocommerce' )) {
		echo '<div class="ktz-cart-wrap login-block">';
			echo '<span class="ktz-cart-head"><a href="' . $woocommerce->cart->get_cart_url() . '"><span class="ktz-cart-button">&nbsp;</span><span class="glyphicon glyphicon-shopping-cart"></span> <span id="top_item_count">' . sprintf(_n('%d', '%d', $woocommerce->cart->cart_contents_count, ktz_theme_textdomain), $woocommerce->cart->cart_contents_count) . '</span></a></span>';
	...
	if ( sizeof( $woocommerce->cart->get_cart() ) > 0 ) :
		...
		echo '<a href="' . $woocommerce->cart->get_cart_url() . '" class="cart-button pull-left"><span class="glyphicon glyphicon-eye-open"></span> ';
